...
def eval_metrics(est, x, y):
    accuracy, f1_score, roc_auc_ovr = get_metrics(y, est.predict(x), est.predict_proba(x))
    return {
        'accuracy': accuracy,
        'f1_score': f1_score,
...
def find_best(
    ...
    with mlflow.start_run():
        ...
        nested_result = cross_validate(
            search, features, target,
            scoring=eval_metrics, cv=cv_outer, n_jobs=n_jobs)
        ...
        mlflow.log_params({
            ...
            'k_folds': 5, 'use_scaler': scale,
            'scaler': scaler, 'normalize': normalize,
            'k_best': k_best
        })
        mlflow.log_param(
            'model_params', ', '.join(
                f'{k}={v}' for k, v in model_kw_fmt.items())
        )
...
def get_space(model: str) -> dict:
    assert model in ('knn', 'forest'), \
        'invalid model %s' % model

    space = {
        'k_best__k': [5, 10, 15, 20, 25, 30, 40, 50, 54]
    }
    if model == 'knn':
        space.update({
